...
### 실행 위치 — GitHub Actions 에서 동작한다 (러너 위치에 따라 막힐 수 있다)

**2026-09-04 스케줄 실행에서 GitHub 호스티드 러너(Azure eastus)로 실제 수집에 성공했다.**
공고 104건을 받아 81건을 선별하고 판정까지 마쳤다. 그러므로 국내 PC 는 **필수가 아니다.**

다만 러너 위치에 따라 막히는 경우가 있다. 하루 전인 2026-09-03 에는 같은 인증키·같은
코드로 아래와 같이 전부 타임아웃이었다.

```
DNS      apis.data.go.kr → 27.101.236.63   정상 해석
TCP      :80 타임아웃    :443 타임아웃
IPv4강제 curl (28) timeout, connect=0.000000s    TCP 핸드셰이크 자체가 안 됨
IPv6     AAAA 레코드 없음                          IPv6 문제 아님
같이 막힘 www.data.go.kr / www.g2b.go.kr
러너      52.165.62.83  Des Moines, Iowa, US (Microsoft Azure)
```

성공한 러너는 eastus, 실패한 러너는 Des Moines 였다.

그 뒤로도 같은 실패가 반복됐다. 정기 수집 실측 기록은 다음과 같다.

| 날짜 | 결과 |
|---|---|
| 2026-09-03 | ❌ 연결 실패 (Des Moines 러너) |
| 2026-09-04 | ✅ 104건 수집 |
| 2026-09-07 | ❌ 연결 실패 |
| 2026-09-08 ~ 09-11 | ✅ 4일 연속 정상 |
| 2026-09-14 ~ 09-16 | ✅ 정상 |
| 2026-09-17 | ❌ 연결 실패 → **재실행 1회로 성공** (같은 커밋, 5분 뒤) |
| 2026-09-18 | ✅ 정상 |
| 2026-09-21 | ✅ 정상 (조회창 4일 첫 적용) |
| 2026-09-22 | ❌ 연결 실패 → 재실행도 실패 → **약 1시간 뒤 3차 시도 성공** |

**15회 중 4회 실패**다(약 27%). 09-17 은 재실행만으로 붙었으니 러너 IP 대역이 고정적으로
차단된 것은 아니다. 그러나 09-22 는 **연달아 두 번 실패하고 약 1시간 뒤에야 붙었다.**
즉 재실행이 곧바로 통하지 않는 날이 있고, 그런 날은 시간을 두고 다시 돌려야 한다.

| 09-22 시도 | 시각(UTC) | 결과 |
|---|---|---|
| 1차 (정기) | 01:20 | ❌ 25개 키워드 전부 연결 실패 |
| 2차 (재실행) | 01:22 | ❌ 동일 |
| 3차 (수동) | 02:20 | ✅ 정상 수집 (11,610B) |

장애 구간이 대략 한 시간 남짓이었다는 뜻이다(관측 3점 기준이라 정확한 길이는 모른다).

**원인은 여전히 확정하지 못했다(확인 필요)** — 조달청 쪽 일시 장애인지, 특정 러너 대역의
문제인지 구분할 근거가 없다. 판단에 쓸 수 있는 관측은 실패율과 재실행 성공 여부뿐이다.

확실한 것은 둘이다 — 인증키·코드는 정상이고, 러너에 따라 도달하지 못할 때가 있다.
실패하면 조용히 넘어가지 않고 **종료코드 1 로 빨간불**이 뜨므로(cb32621) 놓치지 않는다.

따라서 권장 구성은 이렇다.

| 상황 | 할 일 |
|---|---|
| 스케줄 수집이 잘 돌고 있다 | 그대로 둔다. 아무것도 안 해도 된다 |
| 어느 날 `사용 가능한 스킴이 없습니다` / 타임아웃으로 실패한다 | **재실행해 본다** — 실측상 재실행으로 붙는 경우가 있었다(09-17). 재실행도 실패하거나 주 1회꼴로 반복되면 아래 국내 실행 경로로 옮긴다 |

`probe` 명령이 실행 첫머리에 80/443 도달 여부를 찍어 주므로 어느 쪽인지 바로 확인된다.

아래 세 가지는 **국내에서 돌려야 할 때를 위한 대비책**이다. 지금 당장 필요하지는 않다.
...
지정하지 않으면 워크플로가 `cli lookback-days` 로 계산한다 — **평일은 2일, 월요일만 4일**이다.

월요일만 넓히는 이유는 스케줄이 평일에만 돌기 때문이다. 조회창이 늘 2일이면

| 실행 | 조회창(KST) |
|---|---|
| 금요일 08:00 | 수 08:00 ~ 금 08:00 |
| 월요일 08:00 | 토 08:00 ~ 월 08:00 |

가 되어 **금 08:00 ~ 토 08:00 이 어느 실행에도 잡히지 않는다.** 금요일 업무시간에 올라온 공고가
매주 통째로 빠지는 구멍이다. 월요일 창을 4일(목 08:00 부터)로 늘려 덮는다. 3일이 아니라 4일인
것은 GitHub 스케줄이 40~110분씩 늦게 발화해 창의 양끝이 흔들리기 때문이다. 겹치는 구간은
공고번호로 중복 제거되고, 늘어난 이틀이 토·일이라 실제 공고량은 평일 2일 창과 비슷하다.
연속한 실행의 조회창이 겹치는지는 `test_scheduled_windows_cover_every_hour_including_friday`
가 워크플로의 cron 을 직접 읽어 검증한다.
...
| 옵션 | 기본값 | 설명 |
|---|---|---|
| `--days` | 2 | 조회 기간. 실행 누락 대비로 하루보다 넉넉히 잡는다 (Actions 스케줄은 월요일만 4일) |
...
| `사용 가능한 스킴이 없습니다` / 80·443 모두 타임아웃 | 러너에서 조달청 서버에 도달하지 못한 것. 먼저 재실행해 본다. 반복되면 국내 네트워크로 옮긴다 — 5절 «실행 위치» 참고 |
...
tests/              네트워크 없이 도는 단위·통합 테스트 62건
```
