...
성공한 러너는 eastus, 실패한 러너는 Des Moines 였다. 두 번의 관측이라 **원인이
러너 IP 대역인지 그날의 일시적 장애인지는 확정하지 못했다(확인 필요).** 확실한 것은
둘뿐이다 — 인증키·코드는 정상이고, 러너에 따라 도달하지 못할 때가 있다.
...
| 어느 날 `사용 가능한 스킴이 없습니다` / 타임아웃으로 실패한다 | 재실행해 본다. 반복되면 아래 국내 실행 경로로 옮긴다 |
